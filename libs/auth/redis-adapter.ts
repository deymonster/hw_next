...
export function RedisAdapter(client: Redis): Adapter {
	...
	return {
		async createUser(user) {
			...
			await client.hset(
				`user:${id}`,
				adapterUser as unknown as Record<string, string>
			)
			if (adapterUser.email)
				await client.set(`email:${adapterUser.email}`, id)
			return adapterUser
		},
		...
		async updateUser(
			...
			await client.hset(
				`user:${user.id}`,
				updatedUser as unknown as Record<string, string>
			)
			if (updatedUser.email)
				await client.set(`email:${updatedUser.email}`, user.id)
			return updatedUser
		},
		...
		async updateSession(
			...
			const existing = await client.hgetall(
				`session:${session.sessionToken}`
			)
			...
			await client.hset(`session:${session.sessionToken}`, {
				...updatedSession,
