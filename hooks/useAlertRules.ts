...
export function useAlertRules(options?: UseAlertRulesOptions): UseAlertRulesReturn {
    ...
    const { 
        ...
        queryFn: async () => {
            ...
            console.log('[CLIENT] useAlertRules - Alert rules fetched:', result.alertRules?.length || 0)
            return result
        },
        select: (data) => {
    ...
    const createMutation = useMutation({
        mutationFn: (data: CreateAlertRuleRequest) => {
            console.log('[CLIENT] useAlertRules - Creating alert rule:', data.name)
        ...
        onSuccess: (result) => {
            if (result.success && result.alertRule) {
                ...
                options?.onSuccess?.()
                console.log('[CLIENT] useAlertRules - Alert rule created successfully')
            }
        },
        onError: (error) => {
            console.error('[CLIENT] useAlertRules - Error creating alert rule:', error)
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: (result) => {
            if (result.success && result.alertRule) {
                ...
                options?.onSuccess?.()
                console.log('[CLIENT] useAlertRules - Alert rule updated successfully')
            }
        },
        onError: (error) => {
            console.error('[CLIENT] useAlertRules - Error updating alert rule:', error)
            options?.onError?.(error as Error)
        }
    })
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: (_, deletedId) => {
            ...
            options?.onSuccess?.()
            console.log('[CLIENT] useAlertRules - Alert rule deleted successfully')
        },
        onError: (error) => {
            console.error('[CLIENT] useAlertRules - Error deleting alert rule:', error)
            options?.onError?.(error as Error)
        }
    })
    ...
    const toggleMutation = useMutation({
        ...
        onSuccess: (result) => {
            if (result.success && result.alertRule) {
                ...
                options?.onSuccess?.()
                console.log('[CLIENT] useAlertRules - Alert rule toggled successfully')
            }
        },
        onError: (error) => {
            console.error('[CLIENT] useAlertRules - Error toggling alert rule:', error)
            options?.onError?.(error as Error)
        }
    })
    ...
    const importMutation = useMutation({
        ...
        onSuccess: (result) => {
            if (result.success) {
                ...
                queryClient.invalidateQueries({ queryKey: ALERT_RULES_QUERY_KEY })
                options?.onSuccess?.()
                console.log('[CLIENT] useAlertRules - Import successful, imported:', result.imported)
            }
        },
        onError: (error) => {
    ...
    /**
     * Мутация для получения статистики
     */
    const statsMutation = useMutation({
        mutationFn: () => {
            console.log('[CLIENT] useAlertRules - Getting stats')
            return getAlertRulesStats()
        },
        onError: (error) => {
            console.error('[CLIENT] useAlertRules - Error getting stats:', error)
            options?.onError?.(error as Error)
        }
    })
     ...
     */
    const createRuleWithResult = async (data: CreateAlertRuleRequest): Promise<AlertRule> => {
            ...
            createMutation.mutate(data, {
                onSuccess: (result) => {
                    if (result.success && result.alertRule) {
                        ...
                        reject(new Error(result.error || 'Не удалось создать правило алерта'))
                    }
                },
                onError: (error) => {
                    reject(error)
     ...
     */
    const updateRuleWithResult = async (id: string, data: UpdateAlertRuleRequest): Promise<AlertRule> => {
        return new Promise((resolve, reject) => {
            updateMutation.mutate({ id, data }, {
                onSuccess: (result) => {
                    if (result.success && result.alertRule) {
                        ...
                        reject(new Error(result.error || 'Не удалось обновить правило алерта'))
                    }
                },
                onError: (error) => {
                    reject(error)
            ...
            toggleMutation.mutate(id, {
                onSuccess: (result) => {
                    if (result.success && result.alertRule) {
                        ...
                        reject(new Error(result.error || 'Не удалось переключить правило алерта'))
                    }
                },
                onError: (error) => {
                    reject(error)
            ...
            exportMutation.mutate(undefined, {
                onSuccess: (result) => {
                    if (result.success && result.yaml) {
                        ...
                        reject(new Error(result.error || 'Не удалось экспортировать правила алертов'))
                    }
                },
                onError: (error) => {
                    reject(error)
     ...
     */
    const importFromYamlWithResult = async (yamlContent: string): Promise<number> => {
        return new Promise((resolve, reject) => {
            importMutation.mutate(yamlContent, {
                onSuccess: (result) => {
                    if (result.success && typeof result.imported === 'number') {
                        ...
                        reject(new Error(result.error || 'Не удалось импортировать правила алертов'))
                    }
                },
                onError: (error) => {
                    reject(error)
    ...
    /**
     * Получение статистики с возвратом результата
     */
    const getStatsWithResult = async (): Promise<any> => {
        return new Promise((resolve, reject) => {
            statsMutation.mutate(undefined, {
                onSuccess: (result) => {
                    if (result.success && result.stats) {
                        resolve(result.stats)
                    } else {
                        reject(new Error(result.error || 'Не удалось получить статистику'))
                    }
                },
                onError: (error) => {
                    reject(error)
    ...
    const getRule = async (id: string) => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to fetch alert rule'
            console.error('[CLIENT] useAlertRules - Error fetching alert rule:', message)
            options?.onError?.(error as Error)
            return null
        }
    }
    ...
    return {
        // Данные
        alertRules: (alertRulesResponse?.success && alertRulesResponse.alertRules) || [],
        isLoading,
        error,
        ...
        isGettingStats: statsMutation.isPending,
        ...
        statsError: statsMutation.error,

        // Утилиты
        refetch: () => queryClient.invalidateQueries({ queryKey: ALERT_RULES_QUERY_KEY }),
        clearError: () => {
