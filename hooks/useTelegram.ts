...
export function useTelegram() {
    ...
    const verifyBot = useCallback( async (token: string, options?: CallbackOptions)=> {
        if (!user?.id) return null
    ...
    const startBot = useCallback( async (options?: CallbackOptions) => {
        if (!user?.id) return null
    ...
    const sendTestMessage = useCallback(async (options?: CallbackOptions) => {
        if (!user?.id) return null
