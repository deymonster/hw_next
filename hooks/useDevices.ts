...
export function useDevices(options?: UseDevicesOptions) {
    ...
    const fetchDevices = useCallback(async (filters?: DeviceFilterOptions) => {
        ...
        try {
            ...
            console.log('[CLIENT] useDevices - Devices fetched:', result?.length);
            setDevices(result || [])
            return result
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Failed to fetch devices'
            console.error('[CLIENT] useDevices - Error fetching devices:', message);
            setError(message)
    ...
    const forceRefreshDevices = useCallback(async () => {
        ...
        try {
            ...
            console.log(`[CLIENT] useDevices - Received ${freshDevices?.length || 0} devices from server`);
    ...
    const addNewDevice = useCallback(async (data: IDeviceCreateInput) => {
        try {
            console.log('[CACHE] useDevices - Adding new device:', data.name);
            setIsLoading(true)
            const result = await createDevice(data)
            console.log('[CACHE] useDevices - Device created, refreshing device list...');
            await fetchDevices()
            console.log('[CACHE] useDevices - Device list refreshed after adding new device');
            if (options?.onSuccess) {
                options.onSuccess();
            }
            return result
        } catch (error) {
            console.error('[CACHE] useDevices - Error adding new device:', error);
            options?.onError?.(error as Error)
            return null
        } finally {
            setIsLoading(false)
    ...
    const updateIp = useCallback(async (agentKey: string) => {
        try {
            setIsLoading(true)
