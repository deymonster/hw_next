...
start_docker_services() {
        ...
        echo "🐳 Starting Docker services using $(basename "$COMPOSE_FILE")"
        ...
        while ! (cd "$PROJECT_ROOT" && $compose_cmd -f "$COMPOSE_FILE" exec -T postgres pg_isready -U "$pg_user" >/dev/null 2>&1); do
                ...
                if [[ $retries -ge 30 ]]; then
                        echo "⚠️  PostgreSQL did not become ready in time. Continuing anyway." >&2
                        break
                fi
        done
}
...
run_yarn_tasks() {
        if [[ "$SKIP_YARN" == "true" ]]; then
                echo "⏭️  Skipping yarn steps as requested."
                return
        fi

        if ! command -v yarn >/dev/null 2>&1; then
                echo "⚠️  Yarn is not installed; skipping frontend dependency setup."
                return
        fi
...
start_next_dev() {
        if [[ "$START_NEXT" != "true" ]]; then
                echo "ℹ️  Next.js dev server not started automatically. Run 'yarn dev --hostname 0.0.0.0' when ready."
                return
        fi

        if ! command -v yarn >/dev/null 2>&1; then
                echo "⚠️  Yarn is not available; cannot start Next.js dev server automatically."
                return
        fi

        echo "🚀 Starting Next.js development server (Ctrl+C to stop)"
