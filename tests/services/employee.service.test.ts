...
describe('EmployeeService', () => {
        ...
        beforeEach(() => {
                prismaContext = createMockPrisma({
                        departments: [baseDepartment('dept-1', 'IT'), baseDepartment('dept-2', 'Support')],
                        devices: [],
                        employees: []
                })
                service = new EmployeeService(prismaContext.client as unknown as PrismaClient)
        })
        ...
        it('prevents deletion when devices remain assigned', async () => {
                ...
                assert.strictEqual(prismaContext.state.devices[0].employeeId, employee.id)
        })
})
