...
describe('InventoryService', () => {
        ...
        beforeEach(() => {
                ...
                service = new InventoryService(context.client as unknown as PrismaClient)
        })
        ...
        it('removes inventory items and validates existence', async () => {
                const inventory = await service.create({ userId: 'user-1' })
                const item = await service.addItem(inventory.id, { deviceId: 'dev-1' } as any)
        ...
        it('returns latest inventory for user', async () => {
                ...
                const latest = await service.create({ userId: 'user-1', startDate: latestDate })
        ...
        it('fetches all inventories with nested data', async () => {
                ...
                const inventories = (await service.findAllWithItems({
                        include: {
                                items: { include: { device: true, employee: true, department: true } },
                                user: true
                        }
                })) as any
