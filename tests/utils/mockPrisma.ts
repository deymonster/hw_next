...
function matchesWhere<T extends Record<string, any>>(entity: T, where?: WhereInput<T>): boolean {
        if (!where) {
                return true
        }
        ...
        return Object.entries(where).every(([key, value]) => {
                ...
                if (value && typeof value === 'object') {
                        if ('contains' in value) {
                                ...
                                return actual.toLowerCase().includes(String(value.contains).toLowerCase())
                        }
...
function applyOrder<T>(items: T[], orderBy?: Record<string, OrderDirection>): T[] {
        if (!orderBy) {
                        return [...items]
        }
...
function applyEmployeeInclude(
        ...
        if (include.department) {
                result.department =
                        state.departments.find(department => department.id === employee.departmentId) ?? null
        }

        if (include.devices) {
                result.devices = state.devices.filter(device => device.employeeId === employee.id)
        }
...
function applyDepartmentInclude(
        ...
        if (include.employees) {
                const employees = state.employees.filter(emp => emp.departmentId === department.id)
                if (include.employees.select) {
                        result.employees = employees.map(emp => selectFields(emp, include.employees.select))
                } else {
        ...
        if (include.devices) {
                result.devices = state.devices.filter(device => device.departmentId === department.id)
        }
...
function applyInventoryItemInclude(
        ...
        if (include.device) {
                result.device = state.devices.find(device => device.id === item.deviceId) ?? null
        }

        if (include.employee) {
                result.employee = state.employees.find(employee => employee.id === item.employeeId) ?? null
        }

        if (include.department) {
                result.department =
                        state.departments.find(department => department.id === item.departmentId) ?? null
        }
...
function applyInventoryInclude(
        ...
        if (include.items) {
                const items = state.inventoryItems.filter(item => item.inventoryId === inventory.id)
        ...
        if (include.departments) {
                ...
                result.departments = departmentIds
                        .map(id => state.departments.find(department => department.id === id) ?? null)
                        .filter(Boolean)
        }

        if (include.user) {
                result.user = state.users.find(user => user.id === inventory.userId) ?? null
        }

        return result
}

export function createMockPrisma(initial?: CreateMockPrismaArgs): MockPrismaContext {
        const state: MockPrismaState = {
                ...
                inventoryItems: initial?.inventoryItems ? [...initial.inventoryItems] : [],
                notificationSettings: initial?.notificationSettings
                        ? [...initial.notificationSettings]
                        : [],
                users: initial?.users ? [...initial.users] : [],
                inventoryDepartments: new Map(
                        Object.entries(initial?.inventoryDepartments ?? {}).map(([key, value]) => [
                                key,
                                [...value]
                        ])
                )
        ...
        const prismaLike = {
                employee: {
                        findFirst: async ({ where }: { where?: WhereInput<Employee> }) => {
                                return (
                                        state.employees.find(employee => matchesWhere(employee, where)) ?? null
                                )
                        },
                        findUnique: async ({
                                where,
                                ...
                                return employee ? applyEmployeeInclude(employee, include, state) : null
                        },
                        findMany: async ({
                                where,
                                include,
                        ...
                        count: async ({ where }: { where?: WhereInput<Employee> } = {}) => {
                                return state.employees.filter(employee => matchesWhere(employee, where)).length
                        },
                        create: async ({
                                data,
                                include
                        ...
                        update: async ({
                                ...
                                const employee = state.employees.find(emp => emp.id === where.id)
                                if (!employee) {
                        ...
                        delete: async ({
                                ...
                                const index = state.employees.findIndex(emp => emp.id === where.id)
                                if (index === -1) {
                                        throw new Error('Employee not found')
                                }
                ...
                department: {
                        findFirst: async ({ where }: { where?: WhereInput<Department> }) => {
                                return (
                        ...
                        findUnique: async ({
                                ...
                                return (
                                        state.departments.find(department => department.id === where.id) ?? null
                                )
                        },
                        findMany: async ({
                                where,
                        ...
                        update: async ({
                                ...
                                const department = state.departments.find(dep => dep.id === where.id)
                                if (!department) {
                        ...
                        delete: async ({ where }: { where: { id: string } }) => {
                                const index = state.departments.findIndex(dep => dep.id === where.id)
                                if (index === -1) {
                                        throw new Error('Department not found')
                                }
                ...
                device: {
                        findUnique: async ({
                                ...
                                if (include?.employee) {
                                        result.employee =
                                                state.employees.find(employee => employee.id === device.employeeId) ??
                                                null
                                }
                                if (include?.department) {
                                        result.department =
                                                state.departments.find(department => department.id === device.departmentId) ??
                                                null
                                }
                                return result
                        },
                        findFirst: async ({ where }: { where?: WhereInput<Device> }) => {
                                return (
                                        state.devices.find(device => matchesWhere(device, where)) ?? null
                                )
                        },
                        findMany: async ({
                                where,
                                ...
                                return applyOrder(filtered, orderBy).map(device => ({ ...device }))
                        },
                        create: async ({ data }: { data: Partial<Device> }) => {
                                ...
                                const device: Device = {
                                        ...
                                        warrantyPeriod: (data.warrantyPeriod ?? null) as number | null,
                                        ...
                                        activationSig: (data.activationSig ?? null) as string | null,
                                        activationKeyVer: (data.activationKeyVer ?? null) as number | null,
                        ...
                        delete: async ({ where }: { where: { id: string } }) => {
                                const index = state.devices.findIndex(device => device.id === where.id)
                                if (index === -1) {
                                        throw new Error('Device not found')
                                }
                        ...
                        deleteMany: async ({ where }: { where?: WhereInput<Device> }) => {
                                ...
                                state.devices = state.devices.filter(device => !matchesWhere(device, where))
                                return { count: originalLength - state.devices.length }
                        },
                        count: async ({ where }: { where?: WhereInput<Device> } = {}) => {
                                return state.devices.filter(device => matchesWhere(device, where)).length
                        }
                },
                inventory: {
                        findUnique: async ({
                                ...
                                const inventory = state.inventories.find(inv => inv.id === where.id)
                                return inventory ? applyInventoryInclude(inventory, include, state) : null
                        },
                        findMany: async ({
                                where,
                                orderBy,
                        ...
                        findFirst: async ({
                                ...
                                return first ? applyInventoryInclude(first, include, state) : null
                        },
                        create: async ({ data }: { data: Partial<Inventory> & { departments?: { connect: { id: string }[] } } }) => {
                                const now = new Date()
                                ...
                                const connectedDepartments = data.departments?.connect?.map(conn => conn.id) ?? []
                                state.inventoryDepartments.set(inventory.id, connectedDepartments)
                                return { ...inventory }
                        },
                        update: async ({
                                where,
                        ...
                        delete: async ({ where }: { where: { id: string } }) => {
                                const index = state.inventories.findIndex(inv => inv.id === where.id)
                                if (index === -1) {
                                        throw new Error('Inventory not found')
                                }
                ...
                inventoryItem: {
                        ...
                        findFirst: async ({ where }: { where?: WhereInput<InventoryItem> }) => {
                                return (
                                        state.inventoryItems.find(item => matchesWhere(item, where)) ?? null
                                )
                        },
                        delete: async ({ where }: { where: { id: string } }) => {
                                const index = state.inventoryItems.findIndex(item => item.id === where.id)
                                if (index === -1) {
                        ...
                        deleteMany: async ({ where }: { where?: WhereInput<InventoryItem> }) => {
                ...
                notificationSettings: {
                        findUnique: async ({
                                ...
                                const notification = state.notificationSettings.find(setting => {
