...
export async function verifyEmailChangeCode(
	...
	try {
		const changeData = (await services.infrastructure.cache.get(
			`email_change_${userId}`
		)) as EmailChangeData
...
export async function confirmEmailChange(userId: string): Promise<User | null> {
	...
	try {
		...
		if (smtpSettings) {
			console.log(
				'[CONFIRM_EMAIL] Resetting SMTP settings after email change'
			)
...
export async function updateUserPassword(
	...
	try {
		const isPasswordValid = await services.data.user.verifyPassword(
			userId,
			oldPassword
		)

		if (isPasswordValid) {
			const updatedUser = await services.data.user.updatePassword(
				userId,
				newPassword
			)
