...
export async function saveTelegramSettings(
	...
	return telegram_settings.update(userId, {
...
export async function startTelegramBot(userId: string) {
	return telegram.startBot(userId, async (chatId, username, firstName) => {
		await telegram_settings.update(userId, {
...
export async function sendTelegramMessage(userId: string, message: string) {
	...
	return telegram.sendNotification(userId, settings.telegramChatId, message)
}
...
export async function checkTelegramAvailability(userId: string): Promise<{
	...
	try {
		...
		if (!settings.telegramChatId)
			missingFileds.push('chat is not connected')
