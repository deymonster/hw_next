...
export async function getAgentStatuses(ipAddresses: string[]): Promise<{
    ...
    try {
            ...
            try {
                ...
                if (!Array.isArray(status)) {
                    ...
                    try {
                        ...
                        if (device) {
                            await deviceService.updateStatus(device.id, status.up ? 'ACTIVE' : 'INACTIVE');
                        ...
                        console.error(`[AGENT_STATUSES_ACTION] Failed to update device status for ${ip}:`, dbError);
                    }
                }
            } catch (error) {
                console.error(`[AGENT_STATUSES_ACTION] Failed to get status for ${ip}:`, error);
                results[ip] = {
                    health: 'error',
        ...
        console.error('[AGENT_STATUSES_ACTION] Failed to get agent statuses:', error);
        return {
            success: false,
    ...
    try {
        ...
        if (ipAddress && !Array.isArray(status)) {
            try {
                ...
                console.error(`[AGENT_STATUS_ACTION] Failed to update device status:`, error);
            }
        }
        ...
        console.error('[AGENT_STATUS_ACTION] Failed to get agent status:', error);
        return {
            success: false,
