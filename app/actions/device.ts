...
export async function assignDevicesToEmployee({
	...
	try {
		...
		if (employee.departmentId !== departmentId) {
			return {
				success: false,
				error: 'Employee does not belong to this department'
			}
		}
...
export async function updateEmployeeDevicesSelection({
	employeeId,
	departmentId,
	deviceIds
}: {
	...
	try {
		...
		// Берем актуальный департамент, если не передан
		...
		// Что назначить (новые по сравнению с текущими)
		const toAssign = Array.from(nextIds).filter(id => !currentIds.has(id))
		// Что снять (были, но теперь не выбраны)
		const toUnassign = Array.from(currentIds).filter(id => !nextIds.has(id))
