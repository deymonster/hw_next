...
export async function createAlertRule(data: CreateAlertRuleRequest): Promise<{
	...
	try {
		...
		const alertRule = await services.alertRulesManager.createRule(
			data,
			session.user.id
		)

		console.log(
			'[CREATE_ALERT_RULE] Alert rule created successfully:',
			alertRule.id
		)
...
export async function getAlertRules(): Promise<{
	...
	try {
		...
		const alertRules = await services.alertRulesManager[
			'alertRulesService'
...
export async function getAlertRuleById(id: string): Promise<{
	...
	try {
		...
		const alertRule =
			await services.alertRulesManager['alertRulesService'].findById(id)
...
export async function importAlertRulesFromYaml(yamlContent: string): Promise<{
	...
	try {
		...
		const importedRules = await services.alertRulesManager.importRules(
			yamlContent,
			session.user.id
		)

		console.log(
			'[IMPORT_ALERT_RULES] Import successful, imported:',
			importedRules
		)
