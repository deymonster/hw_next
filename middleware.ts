...
function createRedirectUrl(baseUrl: string, from: string, request: NextRequest) {
  const url = new URL(`${AUTH_ROUTES.SIGN_IN}?from=${encodeURIComponent(from)}`, request.url)
...
export default async function middleware(request: NextRequest) {
  ...
  if (token?.id && token?.sessionId) {
    try {
      const response = await fetch(`${request.nextUrl.origin}/api/auth/session-check`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !data || !data.valid) {
        ...
        await fetch(`${request.nextUrl.origin}/api/auth/session-check/delete`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
