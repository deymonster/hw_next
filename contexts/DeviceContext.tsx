...
export function DevicesProvider({ children }: { children: ReactNode }) {
	...
	const setDevicesWithCache = useCallback((newDevices: Device[]) => {
		...
		console.log(
			'[DEVICES_CONTEXT] Devices updated in context, new length:',
			newDevices.length
		)
	}, [])

	const setRefreshDevicesCallback = useCallback(
	...
	const refreshDevicesCallback = useCallback(async () => {
		try {
			...
			console.error(
				'[CACHE] DeviceContext - Error in refreshDevices:',
				error
			)
		} finally {
			setIsLoading(false)
	...
	const forceRefreshCallback = useCallback(async () => {
		try {
			...
			console.error(
				'[CACHE] DeviceContext - Error in force refresh:',
				error
			)
		} finally {
			setIsLoading(false)
