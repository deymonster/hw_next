...
export interface ITelegramSettingsCreateInput extends Omit<TelegramSettings, 'id' | 'createdAt' | 'updatedAt'> {
    userId: string
}
...
export interface ITelegramSettingsRepository {
    update(userId: string, data: Partial<ITelegramSettingsCreateInput>): Promise<TelegramSettings>;
