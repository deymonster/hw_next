...
export class EmailService extends BaseNotificationService {
	...
	private async createTransporter(config: SmtpConfig) {
		console.log('[DEBUG] About to decrypt password with format:', {
			...
			firstPart:
				config.auth.encryptedPassword.split(':')[0]?.substring(0, 10) +
				'...'
		})
	...
	async sendWithConfig(
		...
		try {
			console.log(
				'[SEND_WITH_CONFIG] Starting to send email with config:',
				{
					host: config.host,
					port: config.port,
					secure: config.secure,
		...
		try {
			console.log('[VERIFY_CONNECTION_INPUT]', {
				...
				auth: {
					...
					passwordStart: config.auth.encryptedPassword?.substring(
						0,
						10
					)
				}
